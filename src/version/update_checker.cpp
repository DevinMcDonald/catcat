...
std::optional<std::string> DetectLatestViaBrew() {
    // Wrap each command in a perl alarm so the subprocess is killed after 3s.
    // This gives us a true hard timeout: SIGALRM kills brew, the pipe closes,
    // fread returns 0, and pclose is called immediately — no zombie processes
    // and no blocking from the std::future destructor footgun.
    const std::array<std::string, 2> cmds = {
        "perl -e 'alarm 3; exec @ARGV' -- brew info --json=v2 "
        "devinmcdonald/catcat/catcat 2>/dev/null",
        "perl -e 'alarm 3; exec @ARGV' -- brew info --json=v2 catcat "
        "2>/dev/null",
    };
    for (const auto &cmd : cmds) {
        auto result = FetchLatestForCmd(cmd);
