...
static void RunHeadlessTraining(const std::string &weights_path, bool fresh,
  ...
  while (g_train_running) {
    ...
    const int wi =
        (stats.waves_head.load() + AIStats::kHistLen - 1) % AIStats::kHistLen;
    const float last_waves =
        stats.waves_history[static_cast<std::size_t>(wi)].load();
    const int best_w = stats.best_waves.load();

    const int fi =
        (stats.history_head.load() + AIStats::kHistLen - 1) % AIStats::kHistLen;
    ...
    std::printf("ep=%6d  epm=%5.0f  waves=%5.1f b:%-3d  fit=%6.0f b:%-6.0f  "
                "wr=%4.1f%%  s=%.3f\n",
