...
NeuralNet::NeuralNet(int n_in_, int n_h1_, int n_h2_, int n_out_)
    ...
    auto fill = [&](int offset, int count,
                    std::normal_distribution<float> &dist) {
        for (int i = 0; i < count; ++i) {
            params[static_cast<std::size_t>(offset) +
                   static_cast<std::size_t>(i)] = dist(rng);
        }
    };

    auto d1 = xavier(n_in);
    fill(off_w1(), n_in * n_h1, d1);
    auto d2 = xavier(n_h1);
    fill(off_w2(), n_h1 * n_h2, d2);
    auto d3 = xavier(n_h2);
    fill(off_w3(), n_h2 * n_out, d3);
    // biases stay at zero
}
...
bool NeuralNet::Load(const std::string &path) {
    ...
    if (!f || hdr[0] != n_in || hdr[1] != n_h1 || hdr[2] != n_h2 ||
        hdr[3] != n_out) {
        return false;
    }
