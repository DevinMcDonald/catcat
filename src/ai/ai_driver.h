...
//   sell head    : kCEmb→1
struct CandidateNet {
    static constexpr int kGIn =
        kAIObsGlobal + kAIObsTowerInfo + kAIObsEnemies; // 52
    ...
    static constexpr int kSpec =
        1 + 1 + kAINumTowerTypes; // noop+start+unlock×7 = 9
    static constexpr int kNTypes = kAINumTowerTypes;

    // Cumulative offsets into params[] — public for display-panel
    // introspection.
...
struct AIStats {
    ...
    // Per-type DPS from the most recently completed batch (aggregate damage /
    // total game time)
    ...
    // Fitness history ring buffer (written from training thread, read on
    // render)
    ...
    void PushWinRate(int batch_wins, int batch_total) {
        const float rate = batch_total > 0 ? static_cast<float>(batch_wins) /
    ...
    void PushWaves(float avg_waves, int best_this_batch) {
        ...
        if (c < kHistLen)
            waves_count.store(c + 1);
        if (best_this_batch > best_waves.load())
            best_waves.store(best_this_batch);
    }
...
class Trainer {
  ...
  private:
    EvalResult EvaluatePlayer(const AIPlayer &player,
...
ftxui::Component MakeAIComponent(ftxui::ScreenInteractive &screen,
                                 ...
                                 bool fast_forward = false, bool fresh = false,
                                 int candidates = 8, int eval_games = 10);
