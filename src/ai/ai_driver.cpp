...
CandidateNet::CandidateNet() {
    ...
    auto fill = [&](int off, int count, std::normal_distribution<float> &d) {
        for (int i = 0; i < count; ++i)
            params[static_cast<std::size_t>(off) +
                   static_cast<std::size_t>(i)] = d(rng);
    };
...
std::vector<float> CandidateNet::Forward(const std::vector<float> &obs) const {
    ...
    for (int i = 0; i < kAIObsEnemies; ++i)
        gin[kGlobEnd + i] = obs[static_cast<std::size_t>(kCandEnd) +
    ...
    for (int j = 0; j < kAINumCandidates; ++j) {
        ...
        for (int i = 0; i < kCIn; ++i)
            cin[i] = obs[static_cast<std::size_t>(coff) +
                         static_cast<std::size_t>(i)];
    ...
    for (int t = 0; t < kNTypes; ++t)
        out[static_cast<std::size_t>(kAIActUnlock) +
            static_cast<std::size_t>(t)] = spec[2 + t];
    for (int t = 0; t < kNTypes; ++t)
        for (int j = 0; j < kAINumCandidates; ++j)
    ...
    for (int j = 0; j < kAINumCandidates; ++j)
        out[static_cast<std::size_t>(kAIActUpgrade) +
            static_cast<std::size_t>(j)] = upgrade[j];
    for (int j = 0; j < kAINumCandidates; ++j)
        out[static_cast<std::size_t>(kAIActSell) +
            static_cast<std::size_t>(j)] = sell[j];
    return out;
}
...
Trainer::EvaluatePlayer(const AIPlayer &player,
    ...
    for (int g = 0; g < eval_games_; ++g) {
        ...
        int game_ticks = 0;
        while (!game.IsTerminal() && game_ticks < kMaxGameTicks &&
               running.load()) {
            game.Tick();
...
float Trainer::RunBatch(AIStats &stats, const std::atomic<bool> &running) {
    ...
    for (const auto &cand : candidates)
        futures.push_back(
            std::async(std::launch::async, [this, &cand, &running] {
  ...
  public:
    ...
    bool OnEvent(ftxui::Event event) override {
        ...
        if (event == ftxui::Event::Character('f')) {
            speed_idx_ =
                (speed_idx_ + 1) % static_cast<int>(kDisplaySpeeds.size());
            return true;
        }
  ...
  private:
    ...
    void TickDisplay() {
        ...
        for (int s = 0; s < steps; ++s) {
            ...
            if (display_ticks_since_decision_ >=
                GameAIBridge::kDecisionInterval) {
                display_ticks_since_decision_ = 0;
        ...
        // Accumulate any newly-placed/upgraded towers from the display game
        // since last tick.
    ...
    ftxui::Element RenderStatsSidebar() const {
        ...
        // n_points: how many trailing entries to render (AIStats::kHistLen =
        // all). Returns {top_row, bot_row, most_recent_value}
        struct SparkResult {
            std::string top, bot;
            float cur;
        };
        auto make_spark =
            ...
            return {top_row, bot_row,
                    hist[static_cast<std::size_t>(last)].load()};
        };
        ...
        if (!training_error_.empty()) {
            lines.push_back(text(" TRAINING STOPPED") | bold |
                            color(Color::Red1));
        ...
        lines.push_back(stat_row(
            "Speed", std::to_string(
                         kDisplaySpeeds[static_cast<std::size_t>(speed_idx_)]) +
                         "x  [f]"));
        lines.push_back(
            stat_row("Config", std::to_string(candidates_) + "c × " +
                                   std::to_string(eval_games_) + "g"));
        lines.push_back(
            stat_row("Episodes", std::to_string(stats_.episodes.load())));
        {
            const auto now = std::chrono::steady_clock::now();
            if (std::chrono::duration<float>(now - epm_last_update_).count() >=
                1.f) {
                const float mins =
                    std::chrono::duration<float>(now - stats_.start_time)
                        .count() /
                    60.f;
                cached_epm_ =
                    mins > 0.05f
        ...
        }
        {
            ...
            const int total = w + l;
            std::string val =
                std::to_string(w) + "W / " + std::to_string(l) + "L";
            if (total > 0)
                val += "  (" +
                       fmt(100.f * static_cast<float>(w) /
        ...
        {
            auto [top, bot, cur] =
                make_spark(stats_.sigma_history, stats_.sigma_head, 0.f, 0.5f,
                           stats_.sigma_count.load());
            lines.push_back(
                hbox({text(rpad(" sigma", 13)) | color(Color::GrayLight),
                      text(fmt(cur, 3)) | bold}));
        ...
        lines.push_back(separator());
        {
            auto [top, bot, cur] = make_spark(stats_.win_rate_history,
                                              stats_.win_rate_head, 0.f, 1.f);
            lines.push_back(
        ...
        }
        {
            ...
            const std::string best_str =
                best_w > 0 ? std::to_string(best_w) : "--";
            auto [top, bot, cur] =
                make_spark(stats_.waves_history, stats_.waves_head, 0.f, 100.f,
        ...
        // Load last-batch DPS values and compute efficiency = DPS / cost for
        // color grading.
        ...
        for (int i = 0; i < kAINumTowerTypes; ++i) {
            dps_val[i] =
                stats_.last_batch_dps[static_cast<std::size_t>(i)].load();
        ...
        const float eff_range =
            (max_eff > min_eff) ? (max_eff - min_eff) : 1.0f;
        ...
        for (int i = 0; i < kAINumTowerTypes; ++i) {
            max_cnt = std::max(
                max_cnt,
                stats_.tower_counts[static_cast<std::size_t>(i)].load());
            max_upg = std::max(
                max_upg,
                stats_.upgrade_counts[static_cast<std::size_t>(i)].load());
        }
        const int cnt_w =
        ...
        for (int i = 0; i < kAINumTowerTypes; ++i) {
            ...
            // Upgrades: fixed display width " ★<upg_w digits>" or matching
            // spaces
            ...
            if (dpm > 0.0f) {
                dpm_str = fmt(dpm, 0);
                // Interpolate hue 0°(red)→60°(yellow)→120°(green) based on
                // efficiency rank
                const float t =
                    (max_eff > min_eff)
        ...
        float sell_bias = 0.0f;
        {
            ...
            upg_bias =
                net.params[static_cast<std::size_t>(CandidateNet::kOffUPB)];
            sell_bias =
                net.params[static_cast<std::size_t>(CandidateNet::kOffSLB)];
        }
        ...
        lines.push_back(text(" output bias") | ftxui::bold |
                        color(Color::White));
        for (int t = 0; t < kAINumTowerTypes; ++t)
