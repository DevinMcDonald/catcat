...
class AudioSystem::Impl {
  public:
    ...
    void Update() {
        ...
        if (music_loaded_) {
            if (ma_sound_is_playing(&music_sound_) == MA_FALSE &&
                music_enabled_) {
    ...
    void PlayEvent(const std::string &name) {
        ...
        const size_t idx =
            list.size() == 1 ? 0U
                             : static_cast<size_t>(
                                   dist_(rng_) % static_cast<int>(list.size()));
    ...
    void SetMusicForMap(int map_index) {
        ...
        if (!it->second.intro_files.empty()) {
            ...
            if (intro_path == current_music_path_) {
                shared_intro_ = true;
                current_initial_seek_sec_ = current_intro_start_sec_ >= 0.0F
                                                ? current_intro_start_sec_
                                                : 0.0F;
  ...
  private:
            ...
            for (auto it = j["events"].begin(); it != j["events"].end(); ++it) {
                ...
                if (v.is_array()) {
                    ...
                    if (v.contains("volume") && v["volume"].is_number()) {
                        entry.volume =
                            std::clamp(v["volume"].get<float>(), 0.0F, 2.0F);
                    }
                }
        ...
        if (j.contains("music")) {
            for (auto it = j["music"].begin(); it != j["music"].end(); ++it) {
                ...
                if (v.is_array()) {
                    ...
                    if (v.contains("intro")) {
                        if (v["intro"].is_array()) {
                            for (const auto &f : v["intro"]) {
                                entry.intro_files.push_back(ResolvePath(
                                    base_dir, f.get<std::string>()));
                            }
                        } else if (v["intro"].is_string()) {
                            entry.intro_files.push_back(ResolvePath(
                                base_dir, v["intro"].get<std::string>()));
                        }
                    }
                    if (v.contains("volume") && v["volume"].is_number()) {
                        entry.volume =
                    ...
                    if (v.contains("loop_start") &&
                        v["loop_start"].is_number()) {
                    ...
                    if (v.contains("intro_start") &&
                        v["intro_start"].is_number()) {
    ...
    bool InitIntro(const std::string &path) {
        ...
        ma_uint32 rate = 0;
        ma_sound_get_data_format(&intro_sound_, nullptr, nullptr, &rate,
                                 nullptr, 0);
    ...
    bool StartMainMusic() {
        ...
        ma_uint32 rate = 0;
        ma_sound_get_data_format(&music_sound_, nullptr, nullptr, &rate,
                                 nullptr, 0);
        ma_uint64 length_frames = 0;
